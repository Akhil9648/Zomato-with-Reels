...
const FoodPartnerRegister = () => {
  ...
  const handleSubmit = (e) => {
    ...
    (async () => {
      try {
        console.log('FoodPartnerRegister submit');
        ...
        const payload = {
          name: businessName, // backend expects `name` for business
          contactName,
          phone,
          email,
          password,
        ...
        const response = await api.post('/auth/food-partner/register', payload);

        console.log('partner register success', response.data);
        navigate('/create-food');
      } catch (error) {
        console.error('partner register error', error);
        // Optional: show friendly error toast based on error.response?.data?.message
      }
    })();
  };
